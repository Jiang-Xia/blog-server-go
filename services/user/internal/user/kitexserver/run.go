// Package kitexserver 启动 user-service Kitex 监听并注册到 Nacos。
package kitexserver

import (
	"github.com/Jiang-Xia/blog-server-go/pkg/config"
	"github.com/Jiang-Xia/blog-server-go/pkg/kitexmeta"
	"github.com/Jiang-Xia/blog-server-go/pkg/kitexreg"
	"github.com/Jiang-Xia/blog-server-go/proto/kitex/user/v1/userservice"
	"github.com/cloudwego/kitex/pkg/rpcinfo"
	"github.com/cloudwego/kitex/server"
)

// Run 启动 Kitex 并注册到 Nacos；addr 或 nacos 未配置时不启动。
func Run(cfg *config.Config, srv *Server) (server.Server, error) {
	addr := cfg.Kitex.Addr
	if addr == "" || !cfg.Registry.Enabled() {
		return nil, nil
	}
	r, err := kitexreg.NewRegistry(cfg.Registry)
	if err != nil {
		return nil, err
	}
	tcpAddr, err := kitexreg.ResolveServiceTCPAddr(addr)
	if err != nil {
		return nil, err
	}
	svr := userservice.NewServer(srv,
		server.WithServiceAddr(tcpAddr),
		server.WithRegistry(r),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: config.KitexServiceUser}),
		server.WithMiddleware(kitexmeta.AuthMiddleware),
	)
	go func() {
		if err := svr.Run(); err != nil {
			_ = err
		}
	}()
	return svr, nil
}
