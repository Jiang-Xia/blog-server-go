...
func (s *Service) ReloadCron(ctx context.Context) error {
	...
	for _, t := range tasks {
		cronExpr := normalizeCronExpr(t.Cron)
		if err := s.sched.RegisterTask(t.Name, cronExpr); err != nil && s.log != nil {
...
func (s *Service) CreateTask(ctx context.Context, row *ent.ScheduledTask) (*ent.ScheduledTask, error) {
	...
	if saved.Enabled != 0 && s.sched != nil {
		_ = s.sched.RegisterTask(saved.Name, normalizeCronExpr(saved.Cron))
	}
	return saved, nil
}
...
func (s *Service) StartTask(ctx context.Context, taskName string) (map[string]interface{}, error) {
	...
	if s.sched != nil {
		_ = s.sched.RegisterTask(row.Name, normalizeCronExpr(row.Cron))
	}
...
// normalizeCronExpr 将 Nest 5 段 cron 转为 robfig 6 段（前置秒字段 0）。
