// Package auth 实现 JWT 签发与校验，payload 与 Nest JwtModule 对齐。
package auth
...
func (j *JWTService) SignTriple(id int, nickname, username string, roles []RolePayload) (*TokenTriple, error) {
...
func (j *JWTService) sign(id int, nickname, username string, roles []RolePayload, ttl time.Duration) (string, error) {
	now := time.Now()
	claims := Claims{
		ID:       id,
		Nickname: nickname,
...
func (j *JWTService) RemainingTTL(claims *Claims) int {
