# =============================================================================
# blog-server-go Windows 回滚入口
# 用法：powershell -ExecutionPolicy Bypass -File deploy/pm2/rollback.ps1
#       make rollback
#       make rollback-list
# =============================================================================
...
$envPrefix = "DEPLOY_REMOTE_DIR='$eRemoteDir' DEPLOY_PM2_APPS='$ePm2Apps' DEPLOY_ECOSYSTEM_FILE='$eEcosystemFile'${publicEnv}"
...
Write-Host "==> Rollback: $RemoteDir | PM2: $Pm2Apps"
...
Write-Host '==> Verify'
Invoke-Remote "source ~/.nvm/nvm.sh && nvm use default && DEPLOY_REMOTE_DIR='$eRemoteDir' source /tmp/release-lib.sh && release_pm2_verify_all '$ePm2Apps'"
