...
> **目标**：让 `services/monolith`（`:5000`）具备与四微服务 + gateway（`:8000`）等价的 Nest 替换能力，最终下线 `blog-server`（NestJS）。
>
...
| 部署 | 端口 | 状态 | Nest 对齐度 |
|------|------|------|-------------|
| gateway + user/blog/rpg | `:8000` | **生产推荐** | ✅ ~95%（§3.6 明确不做除外） |
| monolith | `:5000` | **Nest 替换主入口**（Plan 22） | ✅ 与微服务对齐（§3.6 除外） |
| blog-server Nest | `:5000` | 待替换 | 基准 |

### 单体 vs 微服务：缺失功能清单

| # | 能力 | 微服务落点 | 单体现状 | 阻塞切 Nest |
|---|------|-----------|----------|-------------|
...
3. **撤销 deprecated**：Plan 22 验收通过后更新 `services/monolith/README.md`，`:5000` 作为 Nest 替换入口；微服务保留为扩展/学习部署，非阻塞切流。

### 不推荐

- 继续只在微服务开发再手工 cherry-pick 到单体（已导致 G-01/G-02 缺口）
...
| 风险 | 缓解 |
|------|------|
| 双份代码再次漂移 | P22 验收后抽 `pkg/` 或 monolith 复主、微服务从 pkg 引用 |
