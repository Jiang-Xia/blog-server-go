...
- **19 执行计划（01–19）**：Agent/人工逐步执行，每份 ~1–2 周、单一验收点

> 原 5 计划（`02-用户与认证域`、`03-博客内容域`、`04-实时通信与RPG支付`、`05-微服务拆分与生产上线`）已 supersede，由 01–11 替代。  
> **Plan 12–19** 为 v3 交付后与 Nest 差异补齐（M6），在 11 验收通过后按序执行。
...
| M6 Nest差异补齐 | 19 | [19-微信支付补齐.md](./19-微信支付补齐.md) | ~1-2 周 | rpg-service | **可选** |

**总周期**：约 17–20 周（Plan 01–11）+ 约 10–12 周（Plan 12–19，部分可并行）
...
## 不在 Plan 01–11 范围内（已由 Plan 12–19 承接）
...
1. Plan 01→11 按序执行；**Plan 12→19** 在 11 验收通过后执行（**推荐先 17→18，再 12/13**，见上文 M6 顺序表）。
...
| 计划 | 交付文档 |
|------|----------|
| 01–19 | [`docs/{同序号与标题}.md`](../../docs/README.md) |
