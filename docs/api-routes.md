...
> **微服务 gateway**：`http://127.0.0.1:8000`（与单体二选一，**不上生产**）；开发直连见各服务端口表。
...
## 6. monolith 主入口（`:8000`，线上 · 对接 uniapp）

`make dev` / `.\scripts\dev.ps1` 在 `services/monolith` 注册**全部 HTTP 路由**（user / blog / rpg / pay / RAG / 定时任务 / WS），为 **Nest 对等验收与线上部署基准**：
...
> **§1–5** 描述 gateway + 四微服务拆分形态，**仅本地 WSL 学习**；与单体代码不共用，功能可能落后，见 [`nest-parity-matrix.md`](./nest-parity-matrix.md)。详见 [`services/monolith/README.md`](../services/monolith/README.md)。

---

## 7. 维护说明

1. **改路由后须同步本文**：修改 `register_*.go` 或 gateway `app.go` / `proxy/router.go` 时更新对应章节，并执行 `make swag-all` 刷新 OpenAPI（见 [swagger.md](./swagger.md)）。
...
4. **相关文档**：[architecture.md](./architecture.md)、[nest-parity-matrix.md](./nest-parity-matrix.md)、[`services/monolith/README.md`](../services/monolith/README.md)。
